...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 28 15:40:14 2018
...
class pascal_voc_python():  
    root_dir = None
    img_dir = None
    ann_dir = None
    set_dir = None
    def __init__(self, root_dir):
        pascal_voc_python.root_dir = root_dir#'/Users/mprat/personal/VOCdevkit/VOCdevkit/VOC2012/'
        pascal_voc_python.img_dir = os.path.join(root_dir, 'JPEGImages/')
        pascal_voc_python.ann_dir = os.path.join(root_dir, 'Annotations')
        pascal_voc_python.set_dir = os.path.join(root_dir, 'ImageSets', 'Main')
    @staticmethod
    def list_image_sets():
        """
    ...
    def imgs_from_category(cat_name, dataset):
        ...
        """
        filename = os.path.join(pascal_voc_python.set_dir, cat_name + "_" + dataset + ".txt")
        df = pd.read_csv(
            filename,
    ...
    def imgs_from_category_as_list(cat_name, dataset):
        ...
        """
        df = pascal_voc_python.imgs_from_category(cat_name, dataset)
        df = df[df['true'] == 1]
    ...
    def annotation_file_from_img(img_name):
        ...
        """
        return os.path.join(pascal_voc_python.ann_dir, img_name) + '.xml'
    ...
    def load_annotation(img_filename):
        ...
        """
        xml = ""
        with open(pascal_voc_python.annotation_file_from_img(img_filename)) as f:
            xml = f.readlines()
    ...
    def get_all_obj_and_box(objname, img_set):
        ...
        for img in img_list:
            annotation = pascal_voc_python.load_annotation(img)
    ...
    def load_imgs(img_filenames):
        ...
        """
        return np.array([pascal_voc_python.load_img(fname) for fname in img_filenames])
    ...
    def _load_data(category, data_type=None):
        ...
        to_find = category
        filename = os.path.join(pascal_voc_python.root_dir, 'csvs/') + \
            data_type + '_' + \
            category + '.csv'
        if os.path.isfile(filename):
            return pd.read_csv(filename)
        else:
            train_img_list = pascal_voc_python.imgs_from_category_as_list(to_find, data_type)
            data = []
            for item in train_img_list:
                anno = pascal_voc_python.load_annotation(str(item).zfill(6))
    ...
    def get_image_url_list(category, data_type=None):
        ...
        """
        df = pascal_voc_python._load_data(category, data_type=data_type)
        image_url_list = list(
            unique_everseen(list(pascal_voc_python.img_dir + df['fname'])))
    ...
    def get_masks(cat_name, data_type, mask_type=None):
        ...
        df = pascal_voc_python._load_data(cat_name, data_type=data_type)
        # load each image, turn into a binary mask
        masks = []
        prev_url = ""
        blank_img = None
        for row_num, entry in df.iterrows():
            img_url = os.path.join(pascal_voc_python.img_dir, entry['fname'])
            if img_url != prev_url:
                ...
                prev_url = img_url
                img = pascal_voc_python.load_img(img_url)
    ...
    def get_imgs(cat_name, data_type=None):
        ...
        Returns:
            np array of images: np array of loaded images for the category
                and data_type.
        """
        image_url_list = pascal_voc_python.get_image_url_list(cat_name, data_type=data_type)
        imgs = []
        for url in image_url_list:
            imgs.append(pascal_voc_python.load_img(url))
    ...
    def cat_name_to_cat_id(cat_name):
        ...
        """
        cat_list = pascal_voc_python.list_image_sets()
    ...
    def load_data_multilabel(data_type=None):
        ...
        if data_type is None:
            raise ValueError('Must provide data_type = train or val')
        filename = os.path.join(pascal_voc_python.set_dir, data_type + ".txt")
        cat_list = pascal_voc_python.list_image_sets()
        df = pd.read_csv(
            filename,
            delim_whitespace=True,
        ...
        for info in df.itertuples():
            index = info[0]
            fname = info[1]
            anno = pascal_voc_python.load_annotation(fname)
            objs = anno.findAll('object')
            for obj in objs:
                obj_names = obj.findChildren('name')
                for name_tag in obj_names:
